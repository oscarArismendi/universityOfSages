...
async function showReports(){
    ...
    hiddeSecondaryMenus();
    const reportsHTML = document.getElementById("reports");
    ...
    for(tuition of tuitions){
        if(tuition["period_id"] == 1){
            total1 += Number(tuition["price"]);
        }
        else{
            total2 += Number(tuition["price"]);
        }
    }
    ...
    for(tuition of tuitions){
        listSubject[Number(tuition["subject_id"])]++;
    }
